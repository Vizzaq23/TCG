...
- [OPTCG API](https://optcgapi.com/documentation) supplied the older DON!! artwork snapshot. Its `card_image_id` and `DON-001` style archive IDs remain in the source data for provenance. The displayed DON!! selection now uses only exact TCGplayer products from PRB-01 and PRB-02 (60 each), avoiding duplicate artwork-source entries and excluding other DON!! releases.
- `marketplace-catalog.ts` overlays TCGplayer promotional, OP-17, OP-17 release-event and PRB-01/02 DON!! singles from the checked-in TCGCSV export. `archive.ts` applies the displayed DON!! selection and recalculates product counts. Regular OP-17 records gain marketplace links; unresolved promo/parallel identities retain separate source identifiers and full variant names. Their missing artwork and unspecified language remain labeled. See [shared catalog source notes](../../docs/catalog-sources.md) for database sync and preservation details.
